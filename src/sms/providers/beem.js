...
/**
 * Bulk send via /text/multi.
 * Groups rows by message text — all recipients in a group share the same
 * text, sent in one call with to[] as an array (up to MAX_BATCH per call).
...
async function sendBatch({ rows, config, log }) {
  ...
  for (const [message, groupRows] of groups) {
    for (let offset = 0; offset < groupRows.length; offset += MAX_BATCH) {
      ...
      log.debug(`Beem: POST ${c.multiApi} -> ${toList.length} recipient(s)`);
      ...
      // Response has per-recipient results in messages[]; match back by last 9 digits.
      ...
      for (const row of chunk) {
        ...
        // If no per-recipient entry, fall back to overall HTTP 200 success
        const ok = m
          ? /PENDING|ENROUTE|DELIVER|SENT/i.test(groupName)
          : res.ok;
